import { locale } from '@mcschema/core';
import { STATE } from '../app';
import { View } from '../views/View';
import { Toggle } from './Toggle';

export const Errors = (view: View) => `
  <div class="errors">
    ${STATE.errorsHidden.get() ? '' : `
      <div class="error-list">
        ${STATE.errors.get().map(e => `
          <div class="error">
            ...
            <span>-</span>
            <span class="error-message">${locale(e.error, e.params)}</span>
          </div>
        `).join('')}
      </div>
    `}
    ${Toggle(view, [[false, 'chevron_down'], [true, 'issue_opened']], STATE.errorsHidden)}
  </div>
`
